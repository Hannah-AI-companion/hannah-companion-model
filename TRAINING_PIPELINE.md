# Hannah NLP - Language Model Training & Deployment Pipeline

> A complete pipeline for training and deploying a personalized language model (Hannah 360M) with pretraining, SFT (Supervised Fine-Tuning), DPO (Direct Preference Optimization), and a real-time microservices-based backend.

## 🚀 Full System Architecture

Hannah is built using a **microservices-based architecture** to balance latency and response quality (the "Prepared Mind, Fast Response" paradigm).

| Service | Port | Description |
| ------- | ---- | ----------- |
| **Frontend** | 3000 | Web UI for interacting with Hannah. |
| **Backend API** | 8000 | FastAPI orchestrator handling RAG, Audio (TTS/STT), and routing. |
| **Fast Model** | 8001 | Hannah 360M (OLMo) for fast, casual conversation. |
| **Slow Model** | 8002 | Qwen 14B (GGUF) for complex reasoning and deep context. |

### Core Components
- **Model Selector**: A classifier that routes queries to either the Fast or Slow model based on complexity.
- **RAG Component**: Retrieval-Augmented Generation using **ChromaDB** to inject Hannah's personality and facts.
- **Semantic Cache**: Cosine similarity-based caching to skip inference for repeated queries.
- **Audio Pipeline**: Real-time transcription using **Faster-Whisper** and speech synthesis via **Kokoro**.

## Estructura del Proyecto

```
/Hannah
├── backend-hannah/                # Core Deployment (FastAPI, RAG, Model Servers)
├── frontend-hannah/               # Web Interface (HTML/JS)
├── data/                          # Gestión de datos de entrenamiento
...
1. **Install Dependencies**:
   ...
   pip install -r backend-hannah/requirements.txt
   ```

2. **Setup Models**:
   Place model weights in the following directories (not included in repo):
   - `backend-hannah/model/hannah_personality_v2_final.pt`
   - `backend-hannah/models/qwen/hannah-q4_k_m.gguf`

3. **Ingest Knowledge**:
   ```bash
   cd backend-hannah && python rag/ingest_knowledge.py
   ```
